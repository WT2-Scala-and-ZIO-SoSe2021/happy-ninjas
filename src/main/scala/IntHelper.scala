...
object IntHelper {
  ...
  private def greaterThan[T: Ordering](a: T, b: T): Boolean = a > b

  private def lessThan[T: Ordering](a: T, b: T): Boolean = !greaterThan(a, b)
  ...
  def max(arr: Array[Int]): Int = {
    ...
    else filter(arr, greaterThan[Int])
  }

  def min(arr: Array[Int]): Int =
    if (arr.isEmpty) throw new IllegalArgumentException("Array cannot be empty")
    else filter(arr, lessThan[Int])
